...
function updatePresence() {
    if (AppState.currentUser && AppState.currentUser.role === 'student') {
        ...
        if (now - lastPresenceSync > 60000) { 
            ...
            if (uIdx > -1) {
                AppState.data.users[uIdx].lastActive = now;
                AppState.data.users[uIdx].isOnline = true;
                syncData();
            }
        }
    }
...
onValue(dbRef, (snapshot) => {
    ...
    // 🔥 덮어쓰기 시 빈 객체 증발 버그 원천 방어 (Deep Merge)
    if (serverData) {
        AppState.data = { ...DEFAULT_STATE, ...serverData };
        AppState.data.landing = { ...DEFAULT_STATE.landing, ...(serverData.landing || {}) };
        AppState.data.settings = { ...DEFAULT_STATE.settings, ...(serverData.settings || {}) };
    } else {
...
function renderLandingPage() {
    ...
    document.getElementById('ld-f1-b').textContent = ld.f1Badge || ''; document.getElementById('ld-f1-b').className = 'badge-' + (ld.f1Col || 'blue'); document.getElementById('ld-f1-t').textContent = ld.f1Title || ''; document.getElementById('ld-f1-d').textContent = ld.f1Desc || ''; document.getElementById('ld-f1-e').textContent = ld.f1Emoji || '';
    document.getElementById('ld-f2-b').textContent = ld.f2Badge || ''; document.getElementById('ld-f2-b').className = 'badge-' + (ld.f2Col || 'red'); document.getElementById('ld-f2-t').textContent = ld.f2Title || ''; document.getElementById('ld-f2-d').textContent = ld.f2Desc || ''; document.getElementById('ld-f2-e').textContent = ld.f2Emoji || '';
...
function renderStudentChat() {
    ...
    myChats.forEach(c => {
        ...
        if (dateStr !== lastDate) {
            html += `<div style="text-align:center; font-size:11px; color:#94a3b8; margin:10px 0;">${dateStr}</div>`;
            lastDate = dateStr;
        }
...
function renderStudentDashboard() {
    ...
    if (chatBadge) {
        if (unreadChatCount > 0 && !AppState.isChatOpen) { chatBadge.textContent = unreadChatCount; chatBadge.classList.remove('hidden'); }
...
function renderAdminDashboard() {
    ...
    // 🔥 [신규 구현] 관리자 실시간 채팅 탭 연동
    else if (tab === 'chat') {
        const students = (data.users || []).filter(u => u.role === 'student');
        
        // 정렬: 내가 담당하는 학생을 먼저, 그리고 온라인 상태 우선
        students.sort((a,b) => {
            const isMyStudentA = a.teacher === AppState.currentUser.name;
            const isMyStudentB = b.teacher === AppState.currentUser.name;
            if(isMyStudentA && !isMyStudentB) return -1;
            if(!isMyStudentA && isMyStudentB) return 1;
        ...
        let sidebarHtml = '';
        students.forEach(s => {
            const isOnline = s.isOnline && (Date.now() - (s.lastActive||0) < 5*60000); // 5분 이내면 온라인으로 표시
            const unread = (data.chats || []).filter(c => c.receiverId === 'admin' && c.senderId === s.id && !c.read).length;
            
            sidebarHtml += `
            <div class="admin-chat-student-item ${activeId===s.id?'active':''}" data-action="admin-select-chat" data-id="${s.id}">
                <div>
                    <strong style="color:white; display:block; margin-bottom:4px;">${s.name} <span style="font-size:10px; color:#94a3b8;">${s.teacher||''}</span></strong>
                    <span style="font-size:12px; color:${isOnline?'#10b981':'#64748b'}; display:flex; align-items:center; gap:4px;">
                        <span style="display:inline-block; width:8px; height:8px; border-radius:50%; background:${isOnline?'#10b981':'#64748b'};"></span> ${isOnline?'온라인':'미접속'}
                    </span>
                </div>
                ${unread>0 ? `<div style="background:#ef4444; color:white; font-size:10px; font-weight:bold; padding:2px 6px; border-radius:10px;">${unread}</div>` : ''}
            </div>`;
        });
        ...
        if (activeId) {
            ...
            chatHistory.forEach(c => {
                const isAdmin = c.senderId === 'admin';
                const d = new Date(c.timestamp);
                const timeStr = d.toLocaleTimeString('en-US', { hour: 'numeric', minute: '2-digit' });
                msgsHtml += `
                    <div style="display:flex; flex-direction:column; align-items:${isAdmin ? 'flex-end' : 'flex-start'}; margin-bottom:5px;">
                        ${!isAdmin ? `<span style="font-size:11px; color:#64748b; margin-bottom:4px; margin-left:4px;">${activeStu.name}</span>` : ''}
                        <div style="display:flex; align-items:flex-end; gap:5px; flex-direction:${isAdmin ? 'row-reverse' : 'row'};">
            ...
            chatAreaHtml = `
            <div style="padding:15px 20px; border-bottom:1px solid #e2e8f0; background:white; color:#1e293b; font-weight:700; font-size:16px;">
                ${activeStu ? activeStu.name + ' 학생과의 대화' : ''}
            </div>
            <div class="chat-messages" style="background:#f8fafc;" id="admin-chat-messages">${msgsHtml || '<div style="text-align:center; color:#94a3b8; font-size:13px; margin-top:50px;">대화 내역이 없습니다.</div>'}</div>
            <div class="chat-input-area">
                <button class="btn-text" data-action="trigger-admin-chat-file" style="font-size:24px; color:#94a3b8; padding:0;">⊕</button>
                <input type="file" id="admin-chat-file-input" class="hidden" accept="image/*,.pdf">
                <input type="text" id="admin-chat-input-text" placeholder="메시지를 입력해주세요" class="chat-input-field" onkeypress="if(event.key==='Enter') document.getElementById('btn-admin-send-chat').click();">
                <button id="btn-admin-send-chat" class="btn-primary" data-action="send-admin-chat-msg" style="padding:10px; border-radius:50%; width:38px; height:38px; display:flex; justify-content:center; align-items:center;">↑</button>
            </div>
            `;
        }

        html += `
        <div class="admin-chat-layout" style="grid-column:1/-1;">
            <div class="admin-chat-sidebar">${sidebarHtml}</div>
            <div class="admin-chat-area">${chatAreaHtml}</div>
        </div>`;
        
        // 읽음 처리 (관리자 시점)
        let hasUpd = false;
        (data.chats || []).forEach(c => {
            if(c.receiverId === 'admin' && c.senderId === activeId && !c.read) { c.read = true; hasUpd = true; }
        });
        if(hasUpd) syncData();
    }
    else if (tab === 'deploy') {
    ...
    // 관리자 채팅 영역이 그려진 후 스크롤을 맨 아래로 내리기
    if (tab === 'chat') {
        const adminChatMsgs = document.getElementById('admin-chat-messages');
        if (adminChatMsgs) adminChatMsgs.scrollTop = adminChatMsgs.scrollHeight;
    }
}

// 🔥 관리자용 15분 미활동 학생 모니터링
function checkIdleStudents() {
    ...
    AppState.data.users.forEach(u => {
        if(u.role === 'student' && u.isOnline) {
            ...
            if(idleTime > 15 * 60 * 1000) { // 15분
                AppState.idleAlerts.add(u.id);
                alertHtml += `<div class="idle-banner"><span>⚠️ ${u.name} 학생, 15분 이상 학습이 감지되지 않고 있습니다. 확인해주세요.</span></div>`;
            } else {
                if(AppState.idleAlerts.has(u.id)) AppState.idleAlerts.delete(u.id);
            }
        }
    });

    const alertsContainer = document.getElementById('admin-idle-alerts');
    if(alertsContainer) alertsContainer.innerHTML = alertHtml;
}

// 1분(60초)마다 모니터링 검사 실행
...
document.body.addEventListener('change', async (e) => {
    if (e.target.matches('input[type="file"]')) {
        ...
        try {
            ...
            // 🔥 채팅 이미지 첨부
            else if (e.target.id === 'chat-file-input') {
                document.getElementById('chat-file-input').dataset.base64 = b64[0].data;
                document.getElementById('chat-file-input').dataset.filename = b64[0].name;
                showToast(`✅ ${b64[0].name} 첨부됨. 전송을 누르세요.`);
            }
            else if (e.target.id === 'admin-chat-file-input') {
                document.getElementById('admin-chat-file-input').dataset.base64 = b64[0].data;
                document.getElementById('admin-chat-file-input').dataset.filename = b64[0].name;
                showToast(`✅ ${b64[0].name} 첨부됨. 전송을 누르세요.`);
            }
...
document.body.addEventListener('click', (e) => {
    try {
        ...
        if (action === 'nav') switchView(actionNode.dataset.target);
            ...
            if(AppState.currentUser) { 
                // 로그아웃 시 오프라인 처리
                const u = AppState.data.users.find(u => u.id === AppState.currentUser.id);
                if(u) { u.isOnline = false; syncData(); }
                AppState.currentUser = null; localStorage.removeItem('studycampus_session'); showToast("로그아웃 되었습니다."); switchView('landing'); 
            }
        ...
        // 🔥 학생 실시간 채팅 조작
            ...
            const panel = document.getElementById('student-chat-panel');
            AppState.isChatOpen = panel.classList.contains('hidden');
            if(AppState.isChatOpen) {
                panel.classList.remove('hidden');
                // 읽음 처리
                let upd = false;
                (AppState.data.chats || []).forEach(c => { if(c.receiverId === AppState.currentUser.id && !c.read) { c.read = true; upd = true; } });
                if(upd) syncData(); document.getElementById('chat-badge').classList.add('hidden');
                renderStudentChat();
            } else {
        ...
        else if (action === 'send-chat-msg') {
            const input = document.getElementById('chat-input-text'); const fileIn = document.getElementById('chat-file-input');
            ...
            AppState.data.chats.push({
                id: generateId(), senderId: AppState.currentUser.id, receiverId: 'admin',
                text: text, file: b64 ? { data: b64, name: fname } : null,
                timestamp: Date.now(), read: false
            });
            input.value = ''; fileIn.value = ''; delete fileIn.dataset.base64; delete fileIn.dataset.filename;
            syncData(); renderStudentChat();
        }
        ...
        // 🔥 커뮤니티 내글/전체글 토글 기능
            ...
            if(post) { const newText = prompt("게시물 내용을 수정하세요:", post.content); if(newText !== null && newText.trim() !== "") { post.content = newText.trim(); syncData(); showToast("수정되었습니다."); renderStudentDashboard(); } }
        }
        else if (action === 'delete-post') {
            if(confirm("이 게시물을 완전히 삭제하시겠습니까?")) { AppState.data.community = AppState.data.community.filter(c => c.id !== actionNode.dataset.id); if(AppState.activePostId === actionNode.dataset.id) AppState.activePostId = null; syncData(); showToast("삭제되었습니다."); renderStudentDashboard(); }
        }
            ...
            clearLectureTimer(); const lecId = AppState.activeLecture.id; const userIdx = AppState.data.users.findIndex(u => u.id === AppState.currentUser.id); if(!AppState.data.users[userIdx].lectureProgress) AppState.data.users[userIdx].lectureProgress = {}; let prog = AppState.data.users[userIdx].lectureProgress[lecId]?.percent || 0; actionNode.innerHTML = '⏸️'; showToast("수강 기록 시작됨");
            AppState.lectureTimer = setInterval(() => { prog += 5; if (prog >= 90) { prog = 100; clearLectureTimer(); AppState.data.users[userIdx].lectureProgress[lecId] = { percent: 100, done: true }; syncData(); showToast("🎉 90% 달성 완료!"); } else { AppState.data.users[userIdx].lectureProgress[lecId] = { percent: prog, done: false }; } const bar = document.getElementById('live-progress-bar'); const txt = document.getElementById('live-progress-text'); const topTxt = document.getElementById('player-top-progress'); if(bar) bar.style.width = prog + '%'; if(txt) txt.textContent = prog + '%'; if(topTxt) topTxt.textContent = prog + '%'; }, 1000);
        }
        ...
        // 🔥 관리자 채팅 제어
            ...
            if(!AppState.data.chats) AppState.data.chats = [];
            AppState.data.chats.push({
                id: generateId(), senderId: 'admin', receiverId: AppState.adminActiveChatUserId,
                text: text, file: b64 ? { data: b64, name: fname } : null,
                timestamp: Date.now(), read: false
            });
            input.value = ''; fileIn.value = ''; delete fileIn.dataset.base64; delete fileIn.dataset.filename;
