...
function renderLecturePlayer() {
    ...
    <div class="lecture-player-grid">
        <div class="player-main">
            ...
            <h1 style="font-size:24px; font-weight:800; margin-bottom:15px;">${activeLecture.title}</h1>
    ...
    // 타이머 자동 가동 (플레이어 진입 시 자동 퍼센트 증가 시뮬레이션)
    if (!isDone) {
        clearLectureTimer();
        let currentProg = prog.percent;
        AppState.lectureTimer = setInterval(() => {
            currentProg += 2; // 2%씩 증가 시뮬레이션
            ...
            if (currentProg >= 90) { 
                ...
                renderLecturePlayer(); // UI 갱신
            } 
            else { 
                AppState.data.users[userIdx].lectureProgress[activeLecture.id] = { percent: currentProg, done: false }; 
            }
...
function renderStudentDashboard() {
    ...
    // 🔥 [완벽 반영] 스크린샷 1번처럼 화이트톤 카드형태 리스트로 변형
        ...
        <div style="background:white; border-radius:16px; padding:0; overflow:hidden; color:#1e293b; margin-bottom:20px; box-shadow:0 10px 20px rgba(0,0,0,0.2);">
            <div style="background:#2563eb; padding:30px 20px; color:white;">
                <div style="font-size:12px; font-weight:700; margin-bottom:5px; opacity:0.8;">2026년 정규반</div>
                <h2 style="font-size:24px; font-weight:800; margin-bottom:5px; letter-spacing:-1px;">기출제로원</h2>
                <div style="font-size:13px; opacity:0.8;">현재 주차 진행중</div>
            </div>
            <div style="padding:20px;">
                <h3 style="font-size:16px; font-weight:800; margin-bottom:15px; display:flex; align-items:center; gap:8px;"><span style="color:#2563eb;">●</span> 전체 강의 목록</h3>
                <div style="display:flex; flex-direction:column; gap:0;">
        `;
        if (!data.lectures || data.lectures.length === 0) {
            html += `<div style="text-align:center; padding:40px; color:#94a3b8; font-weight:600;">등록된 강의가 없습니다.</div>`;
        } else {
            data.lectures.forEach((l, idx) => {
                const p = me.lectureProgress?.[l.id] || { percent: 0, done: false };
                ...
                <div style="display:flex; justify-content:space-between; align-items:center; padding:15px 0; border-bottom:1px solid #f1f5f9;">
                    <div style="display:flex; align-items:center; gap:15px;">
                        <div style="width:40px; height:40px; border-radius:50%; background:${iconBg}; color:${iconColor}; display:flex; justify-content:center; align-items:center; font-size:16px; font-weight:bold;">${iconChar}</div>
                        <div>
                            <div style="font-weight:700; font-size:15px; margin-bottom:4px; color:#0f172a;">${idx+1}강. ${l.title}</div>
                            <div style="font-size:12px; color:#64748b;">${p.percent}% 수강중</div>
                        </div>
                    </div>
                    <button class="btn-text" style="color:${btnColor}; font-weight:800; font-size:14px; background:#f8fafc; padding:6px 14px; border-radius:20px;" data-action="open-lecture" data-id="${l.id}">${btnText}</button>
                </div>
                `;
            });
        }
        ...
        if (action === 'nav') switchView(actionNode.dataset.target);
            ...
            if(drop) {
                ...
                if(!drop.classList.contains('hidden')) { 
                    let upd = false; (AppState.data.alerts||[]).forEach(a => { if(a.studentId === AppState.currentUser.id && !a.read) { a.read = true; upd = true; }}); 
...
document.body.addEventListener('submit', async (e) => {
    ...
    try {
        if(e.target.id === 'form-login') {
            ...
            if(!AppState.data.settings) AppState.data.settings = {}; 
            if(e.target.id === 'form-admin-system') { AppState.data.settings.dashBanner = document.getElementById('set-dash-banner').value; AppState.data.settings.popup = { active: document.getElementById('set-pop-active').checked, tag: document.getElementById('set-pop-tag').value, title: document.getElementById('set-pop-title').value, desc: document.getElementById('set-pop-desc').value, b1Title: document.getElementById('set-pop-b1-t').value, b1Desc: document.getElementById('set-pop-b1-d').value, b2Title: document.getElementById('set-pop-b2-t').value, b2Desc: document.getElementById('set-pop-b2-d').value, btnUrl: document.getElementById('set-pop-btn-url').value }; }
            if(e.target.id === 'form-admin-settings') {
                AppState.data.landing = { heroSub: document.getElementById('set-h-sub').value, heroTitle: document.getElementById('set-h-tit').value.replace(/\n/g, '<br>'), heroDesc: document.getElementById('set-h-desc').value, s1Num: document.getElementById('set-s1-n').value, s1Txt: document.getElementById('set-s1-t').value, s2Num: document.getElementById('set-s2-n').value, s2Txt: document.getElementById('set-s2-t').value, s3Num: document.getElementById('set-s3-n').value, s3Txt: document.getElementById('set-s3-t').value, secTitle: document.getElementById('set-sec-tit').value, f1Badge: document.getElementById('set-f1-b').value, f1Col: document.getElementById('set-f1-c').value, f1Title: document.getElementById('set-f1-t').value, f1Desc: document.getElementById('set-f1-d').value, f1Emoji: document.getElementById('set-f1-e').value, f2Badge: document.getElementById('set-f2-b').value, f2Col: document.getElementById('set-f2-c').value, f2Title: document.getElementById('set-f2-t').value, f2Desc: document.getElementById('set-f2-d').value, f2Emoji: document.getElementById('set-f2-e').value, botTitle: document.getElementById('set-b-tit').value };
            }
