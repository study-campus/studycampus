...
const DEFAULT_STATE = {
    ...
    settings: {
        ...
        popup: { 
            active: false, tag: "이용 안내", title: "함께 발전해 나가요!", desc: "의견을 남겨주시면 큰 도움이 됩니다.", 
            b1Title: "카카오톡 채널 친구 추가하기", b1Desc: "1개월 무료 수강권을 드립니다.", b1Url: "http://pf.kakao.com/_xdxnxfXX",
            b2Title: "인스타그램 팔로우하기", b2Desc: "카카오톡 채널에 바로가기에 있습니다.", b2Url: "https://instagram.com" 
        }
    },
...
function switchAuthMode(mode) {
    ...
    document.getElementById('modal-find-id').classList.add('hidden');
...
function checkAndShowPopup() {
    ...
    let btnsHtml = '';
    if(setPopup.b1Title) {
        btnsHtml += `<div class="pop-action-item" onclick="window.open('${setPopup.b1Url||'#'}', '_blank')"><div class="icon">💬</div><div class="text"><h4>${setPopup.b1Title}</h4><p>${setPopup.b1Desc}</p></div></div>`;
    }
    if(setPopup.b2Title) {
        btnsHtml += `<div class="pop-action-item" onclick="window.open('${setPopup.b2Url||'#'}', '_blank')"><div class="icon">📞</div><div class="text"><h4>${setPopup.b2Title}</h4><p>${setPopup.b2Desc}</p></div></div>`;
    }
    
    const btnArea = document.getElementById('pop-btn-area');
    if(btnArea) btnArea.innerHTML = btnsHtml;
...
function renderStudentDashboard() {
    ...
    // 🔥 마이페이지: 회원정보 수정 모달 호출 버튼 및 D-Day 수강권 연동 추가
        ...
        if (me.ticketExpiry) {
            ...
            expiryHtml = `
                <div>
                    <div style="font-size:12px; color:#94a3b8; margin-bottom:4px;">수강권</div>
                    <div style="font-size:18px; font-weight:bold; color:white;">${expD.getMonth()+1}/${expD.getDate()}(${days[expD.getDay()]})까지 <span style="color:#3b82f6; margin-left:10px;">${dStr}</span></div>
                </div>
            `;
        }

        html += `
            <div style="border:1px solid #1e293b; background:var(--bg-card); border-radius:12px; padding:20px; display:flex; justify-content:space-between; align-items:center; margin-bottom:20px;">
                ${expiryHtml}
                <button class="btn-primary btn-sm" style="background:#f1f5f9; color:#0f172a; padding:8px 16px; border-radius:20px;" data-action="nav" data-target="payment">결제 및 연장</button>
            </div>
            <div class="stu-card">
            ...
            <form id="form-admin-system" style="display:grid; grid-template-columns:1fr 1fr; gap:20px;">
                ...
                <div style="background:var(--bg-dark); padding:20px; border-radius:12px; grid-column:1/-1;">
                    ...
                    <div style="display:grid; grid-template-columns:1fr 1fr; gap:15px;">
                        <input type="text" id="set-pop-tag" class="admin-input" value="${pop.tag}"><input type="text" id="set-pop-title" class="admin-input" value="${pop.title}">
                        <textarea id="set-pop-desc" class="admin-input" style="grid-column:1/-1; height:60px;">${pop.desc}</textarea>
                        ...
                        <div style="background:#1e293b; padding:15px; border-radius:8px; border:1px solid var(--border);">
                            ...
                            <input type="text" id="set-pop-b1-t" class="admin-input" value="${pop.b1Title}"><input type="text" id="set-pop-b1-d" class="admin-input" value="${pop.b1Desc}"><input type="text" id="set-pop-b1-url" class="admin-input" value="${pop.b1Url||''}" placeholder="이동할 링크">
                        </div>
                        
                        <div style="background:#1e293b; padding:15px; border-radius:8px; border:1px solid var(--border);">
                            <h4 style="margin-bottom:10px; font-size:13px; color:#94a3b8;">박스 2 내용 (아래)</h4>
                            <input type="text" id="set-pop-b2-t" class="admin-input" value="${pop.b2Title}"><input type="text" id="set-pop-b2-d" class="admin-input" value="${pop.b2Desc}"><input type="text" id="set-pop-b2-url" class="admin-input" value="${pop.b2Url||''}" placeholder="이동할 링크">
                        </div>
                    </div>
                </div>
...
document.body.addEventListener('click', (e) => {
    try {
        ...
        // 🔥 알림 드롭다운 컨트롤
            ...
            AppState.lectureTimer = setInterval(() => {
                currentProg += 5; 
                if (currentProg >= 90) { currentProg = 100; clearLectureTimer(); AppState.data.users[userIdx].lectureProgress[lecId] = { percent: 100, done: true }; syncData(); showToast("🎉 90% 달성 완료!"); renderLecturePlayer(); } 
        ...
        else if (action === 'delete-account') { if(confirm("탈퇴하시면 현재까지 모든 데이터가 삭제되며 되돌릴 수 없습니다. 정말 탈퇴하시겠습니까?")) { AppState.data.users = AppState.data.users.filter(u => u.id !== AppState.currentUser.id); syncData(); localStorage.removeItem('studycampus_session'); AppState.currentUser = null; switchView('landing'); } }
        ...
        // 🔥 결제 승인 시 자동으로 일수 계산 연동
            ...
            if(p) { 
                ...
                if(u) {
                    ...
                    if(currentExp < new Date()) currentExp = new Date(); // 만료되었으면 오늘부터 계산
        ...
        // 🔥 [AI 리포트 개인화] 학생 데이터 기반 맞춤형 텍스트 생성기
            ...
            let hwEval = hwSub.length >= hwTotal ? "모든 과제를 성실하게 수행하고 있습니다." : "일부 과제가 누락되어 꾸준한 제출 관리가 필요합니다.";
            ...
            AppState.data.alerts.unshift({ id: generateId(), studentId: AppState.aiModal.studentId, title: "주간 분석 리포트 도착", content: "나의 새로운 맞춤 학습 리포트가 등록되었습니다.", read: false, date: new Date().toISOString().split('T')[0] });
    ...
    try {
        if(e.target.id === 'form-login') {
            ...
            if(id === 'studycampus' && pw === 'studycampus26') { AppState.currentUser = { id: 'admin', name: '최고관리자', role: 'admin' }; localStorage.setItem('studycampus_session', JSON.stringify(AppState.currentUser)); switchView('admin'); } 
                ...
                if(user && user.active && (!user.pw || user.pw === pw)) { AppState.currentUser = user; localStorage.setItem('studycampus_session', JSON.stringify(AppState.currentUser)); switchView('student'); } 
        ...
        // 🔥 비밀번호 찾기 (변경) 기능
            ...
            if(userIdx > -1) { 
                ...
                AppState.currentUser = AppState.data.users[userIdx]; localStorage.setItem('studycampus_session', JSON.stringify(AppState.currentUser)); 
            } 
...
// 🔥 [완벽 반영] 사이트 재접속 시 자동 로그아웃 (페이지 새로고침 시 무조건 랜딩페이지로 이동)
document.addEventListener('DOMContentLoaded', () => {
    localStorage.removeItem('studycampus_session'); // 항상 세션 초기화
    AppState.currentUser = null;
    
    document.querySelectorAll('.global-element').forEach(el => el.classList.remove('hidden')); // 랜딩 뷰는 기본 노출
    switchView('landing');
});
